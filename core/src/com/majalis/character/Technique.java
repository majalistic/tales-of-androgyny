...
public class Technique {
	...
	private boolean isEvaded(TechniquePayload otherPayload) { return technique.getTechniqueHeight() != TechniqueHeight.NONE && otherPayload.getEvasion() >= 4; }
	...
	public Array<Attack> resolve(Technique otherTechnique) {
		...
		boolean isSuccessful = validTarget(otherTechnique) && validTechniqueHeight(otherTechnique);
		...
		Array<Attack> resultingAttacks = new Array<Attack>(new Attack[]{new Attack(
			isEvaded(otherPayload) ? Status.EVADED : fizzle ? Status.FIZZLE : isSuccessful ? (blockMod < 1 ? (otherPayload.getBlock() > 0 ? Status.BLOCKED : Status.PARRIED) : Status.SUCCESS) : failure ? Status.FAILURE : Status.MISSED, 
			technique.getName(), 
			thisPayload.getDamage(), // this is the damage actually dealt - need raw damage before that as well
			blockMod,
