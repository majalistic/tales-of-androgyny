...
public class PlayerCharacter extends AbstractCharacter {
	...
	private static ObjectSet<Techniques> getBaseTechniques() {
		...
		baseTechniques.addAll(
			DO_NOTHING, POWER_ATTACK, TEMPO_ATTACK, RESERVED_ATTACK, DUCK, SPRING_ATTACK, NEUTRAL_ATTACK, REVERSAL_ATTACK, CAREFUL_ATTACK, BLOCK, GUARD, KICK_OVER_FACE_UP, KICK_OVER_FACE_DOWN,
	...
	private Array<Techniques> getPossibleKnownTechniques(AbstractCharacter target) {
		...
		switch(stance) {
			...
			case BALANCED:
				...
				if (target.getStance() == Stance.PRONE) {
					possibles.addAll(getTechniques(KICK_OVER_FACE_UP));
				}
				else if (target.getStance() == Stance.SUPINE) {
					possibles.addAll(getTechniques(KICK_OVER_FACE_DOWN));
				}
				if (canSitOn(target)) {
					possibles.addAll(getTechniques(SIT_ON_IT, TURN_AND_SIT));
				}
